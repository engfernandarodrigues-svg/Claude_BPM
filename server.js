...
if (!API_KEY) {
  console.error('Uso: node server.js sk-ant-api03-SUA_CHAVE');
  process.exit(1);
}

const PORT = 3131;

// Intercepta só a chamada do BPMN e melhora o prompt
...
function anthropicRequest(body, res) {
...
const server = http.createServer(function(req, res) {
  ...
  if (req.method === 'POST' && req.url === '/v1/messages') {
    ...
    req.on('end', function() {
      try { anthropicRequest(JSON.parse(body), res); }
      catch(e) { res.writeHead(400); res.end(JSON.stringify({error:{message:'JSON invalido'}})); }
    });
    return;
  }
...
server.listen(PORT, function() {
  ...
  console.log('    Abra o agente_bpm.html no browser e clique em Gerar com IA');
});
